...
// (Per-anomaly TikTok-only inline helper retired in favour of the
// shared inlineAdThumbnails from _pulseShared.js. The shared helper
// runs once on the adsByCampaign map BEFORE buildHtml, inlines every
// platform's thumbnail in one sweep, and the rendered anomalies just
// read the already-inlined data URLs from topAd.thumbnail. Net
// effect: no brittle CDN URLs in any Pulse email regardless of
// platform.)
...
function buildHtml(opts) {
  ...
  return '<!DOCTYPE html>' +
    ...
    '<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="max-width:720px;background:linear-gradient(170deg,' + P.panel + ' 0%,' + P.panel2 + ' 100%);border-radius:22px;overflow:hidden;border:1px solid ' + P.rule + ';">' +
      ...
      '</div>' +
      '<div style="font-size:11px;color:' + P.ember + ';letter-spacing:6px;font-weight:800;margin-top:0;margin-bottom:8px;text-transform:uppercase;font-family:Manrope,Helvetica,Arial,sans-serif;">Daily Pulse</div>' +
