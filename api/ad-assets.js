...
import { isLeadAction, isAppInstallAction } from "./_pulseShared.js";
...
function resultsFromActions(actions) {
  ...
  // Per-ad Math.max across alias action_types via shared helpers, so
  // Meta surfacing one conversion under multiple types at once isn't
  // double-counted AND new lead variants Meta ships are picked up
  // automatically (the isLeadAction heuristic has a catch-all for any
  // action_type containing "lead" minus install/video/post).
  actions.forEach(function(a) {
    var t = String(a.action_type || "").toLowerCase();
    var v = parseFloat(a.value || 0) || 0;
    if (isLeadAction(t)) out.leads = Math.max(out.leads, v);
    else if (isAppInstallAction(t)) out.installs = Math.max(out.installs, v);
