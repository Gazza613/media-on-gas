  ...
  return [
    {
      ...
      prompt: [
        "Run the GAS Guided Campaign Build using the BRIEF_CARD flow. Do NOT run a Q&A. Your first substantive message MUST be a single BRIEF_CARD, prefilled from whatever the AM already said in their opening turn PLUS whatever the injected client memory covers.",
        "",
        "If the opening turn already includes a client, extract every field you can and emit the BRIEF_CARD immediately. Otherwise ask ONE short question, 'Which client are we building for?', wait for the answer, then emit the BRIEF_CARD.",
        "",
        "See the BRIEF_CARD (STRUCTURED BRIEF) section of your system prompt for the exact card format, field list, and BRIEF_FILLED handling. Fall back to the 15-turn Q&A ONLY if the AM explicitly says they prefer that (rare).",
        "",
