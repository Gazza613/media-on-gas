...
function buildSystemPrompt() {
  return [
    ...
    "  1. List the folder contents. NEVER tell the user 'the engine cannot list folders' before you have tried the correct operation. Recipes:",
    "     - GOOGLE DRIVE folder link like https://drive.google.com/drive/folders/<FOLDER_ID>?usp=sharing → extract FOLDER_ID (the string between /folders/ and the ? or end of URL) then call the engine's Google Drive search_files operation with the query \"'<FOLDER_ID>' in parents and trashed=false\" (note the single-quotes around the folder id and the trashed=false filter). Iterate pageToken until every file returned. That is the correct listing operation, do not confuse it with copy_file, list_recent_files (which ignores the folder), or 'upload only' variants. If find_operations does not surface search_files, try get_operation_inputs for it directly. If it truly is not present, THEN fall back to step 3.",
    "     - DROPBOX folder link like https://www.dropbox.com/scl/fo/... → use the Dropbox list-folder operation with the shared-folder path. Public shared links resolve without extra auth.",
