...
if start < 0 or end < 0:
    ...
    new_obj = """{/* OBJECTIVE ASSESSMENT */}
          <div style={{background:P.glass,borderRadius:18,padding:"6px 24px 24px",marginBottom:28,border:"1px solid "+P.rule}}>
            ...
            {(function(){
              ...
              var getResultLabel=function(obj){
                if(obj==="Leads")return "Leads";
                if(obj==="Follows")return "Follows";
                if(obj==="Page Likes")return "Likes";
                return "Clicks";
              };

              var getCostLabel=function(obj){
                if(obj==="Leads")return "CPL";
                if(obj==="Follows")return "CPF";
                if(obj==="Page Likes")return "CPL";
                return "CPC";
              };
              ...
              var rows=sel.map(function(camp){
                ...
                var spend=parseFloat(camp.spend||0);
                var clicks=parseFloat(camp.clicks||0);
                var costPer=result>0?spend/result:0;
                var convRate=clicks>0&&obj==="Leads"?(parseFloat(camp.leads||0)/clicks*100):0;
              ...
              var groups={};
              objectives.forEach(function(o){groups[o]=rows.filter(function(r){return r.objective===o;});});
              var trafficRows=rows.filter(function(r){return r.objective==="Traffic";});
              if(trafficRows.length>0){
                groups["Landing Page Clicks"]=groups["Landing Page Clicks"].concat(trafficRows);
              }
              ...
              objectives.forEach(function(objName){
                var g=groups[objName];
                if(!g||g.length===0)return;
                var totalSpend=g.reduce(function(a,r){return a+r.spend;},0);
                var totalClicks=g.reduce(function(a,r){return a+r.clicks;},0);
                var totalResults=g.reduce(function(a,r){return a+r.result;},0);
                var totalCostPer=totalResults>0?totalSpend/totalResults:0;
                var totalConv=totalClicks>0&&objName==="Leads"?(totalResults/totalClicks*100):0;
              ...
              if(sections.length===0){
                sections.push(<div key="none" style={{padding:30,textAlign:"center",color:P.dim,fontFamily:fm}}>No objective data found for the selected campaigns in this date range.</div>);
              }

              var allTotals={spend:rows.reduce(function(a,r){return a+r.spend;},0),clicks:rows.reduce(function(a,r){return a+r.clicks;},0),leads:rows.filter(function(r){return r.objective==="Leads";}).reduce(function(a,r){return a+r.result;},0),follows:rows.filter(function(r){return r.objective==="Follows";}).reduce(function(a,r){return a+r.result;},0),pageLikes:rows.filter(function(r){return r.objective==="Page Likes";}).reduce(function(a,r){return a+r.result;},0),appClicks:rows.filter(function(r){return r.objective==="App Store Clicks";}).reduce(function(a,r){return a+r.result;},0),lpClicks:rows.filter(function(r){return r.objective==="Landing Page Clicks"||r.objective==="Traffic";}).reduce(function(a,r){return a+r.result;},0)};
              var leadSpend=rows.filter(function(r){return r.objective==="Leads";}).reduce(function(a,r){return a+r.spend;},0);
              var followSpend=rows.filter(function(r){return r.objective==="Follows";}).reduce(function(a,r){return a+r.spend;},0);
              var likeSpend=rows.filter(function(r){return r.objective==="Page Likes";}).reduce(function(a,r){return a+r.spend;},0);
              var appSpend=rows.filter(function(r){return r.objective==="App Store Clicks";}).reduce(function(a,r){return a+r.spend;},0);
              var lpSpend=rows.filter(function(r){return r.objective==="Landing Page Clicks"||r.objective==="Traffic";}).reduce(function(a,r){return a+r.spend;},0);

              return <div>
                {sections}

                <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:12,marginBottom:16}}>
                  <Glass accent={P.fb} hv={true} st={{padding:16,textAlign:"center"}}><div style={{fontSize:8,color:P.dim,fontFamily:fm,letterSpacing:2,marginBottom:4}}>APP STORE CLICKS</div><div style={{fontSize:22,fontWeight:900,color:P.fb,fontFamily:fm}}>{fmt(allTotals.appClicks)}</div><div style={{fontSize:9,color:P.dim,fontFamily:fm,marginTop:4}}>CPC: {fR(allTotals.appClicks>0?appSpend/allTotals.appClicks:0)}</div></Glass>
                  <Glass accent={P.cyan} hv={true} st={{padding:16,textAlign:"center"}}><div style={{fontSize:8,color:P.dim,fontFamily:fm,letterSpacing:2,marginBottom:4}}>LP CLICKS</div><div style={{fontSize:22,fontWeight:900,color:P.cyan,fontFamily:fm}}>{fmt(allTotals.lpClicks)}</div><div style={{fontSize:9,color:P.dim,fontFamily:fm,marginTop:4}}>CPC: {fR(allTotals.lpClicks>0?lpSpend/allTotals.lpClicks:0)}</div></Glass>
                  <Glass accent={P.rose} hv={true} st={{padding:16,textAlign:"center"}}><div style={{fontSize:8,color:P.dim,fontFamily:fm,letterSpacing:2,marginBottom:4}}>LEADS</div><div style={{fontSize:22,fontWeight:900,color:P.rose,fontFamily:fm}}>{fmt(allTotals.leads)}</div><div style={{fontSize:9,color:P.dim,fontFamily:fm,marginTop:4}}>CPL: {fR(allTotals.leads>0?leadSpend/allTotals.leads:0)}</div></Glass>
                  <Glass accent={P.tt} hv={true} st={{padding:16,textAlign:"center"}}><div style={{fontSize:8,color:P.dim,fontFamily:fm,letterSpacing:2,marginBottom:4}}>FOLLOWS</div><div style={{fontSize:22,fontWeight:900,color:P.mint,fontFamily:fm}}>{fmt(allTotals.follows)}</div><div style={{fontSize:9,color:P.dim,fontFamily:fm,marginTop:4}}>CPF: {fR(allTotals.follows>0?followSpend/allTotals.follows:0)}</div></Glass>
                  <Glass accent={P.fb} hv={true} st={{padding:16,textAlign:"center"}}><div style={{fontSize:8,color:P.dim,fontFamily:fm,letterSpacing:2,marginBottom:4}}>PAGE LIKES</div><div style={{fontSize:22,fontWeight:900,color:P.fb,fontFamily:fm}}>{fmt(allTotals.pageLikes)}</div><div style={{fontSize:9,color:P.dim,fontFamily:fm,marginTop:4}}>CPL: {fR(allTotals.pageLikes>0?likeSpend/allTotals.pageLikes:0)}</div></Glass>
                </div>

                <Insight title="Objective Performance" accent={P.rose} icon={Ic.target(P.rose,16)}>{(function(){var p=[];p.push("The selected campaigns invested "+fR(allTotals.spend)+" generating "+fmt(allTotals.clicks)+" total clicks across all objectives.");if(allTotals.appClicks>0)p.push("App Store click campaigns delivered "+fmt(allTotals.appClicks)+" clicks at "+fR(appSpend/allTotals.appClicks)+" CPC.");if(allTotals.lpClicks>0)p.push("Landing page campaigns drove "+fmt(allTotals.lpClicks)+" clicks at "+fR(lpSpend/allTotals.lpClicks)+" CPC.");if(allTotals.leads>0)p.push("Lead generation campaigns produced "+fmt(allTotals.leads)+" leads at "+fR(leadSpend/allTotals.leads)+" cost per lead"+(allTotals.clicks>0?", a "+(allTotals.leads/rows.filter(function(r){return r.objective==="Leads";}).reduce(function(a,r){return a+r.clicks;},0)*100).toFixed(1)+"% conversion rate.":"."));if(allTotals.follows>0)p.push("Follower campaigns acquired "+fmt(allTotals.follows)+" new followers at "+fR(followSpend/allTotals.follows)+" cost per follow.");if(allTotals.pageLikes>0)p.push("Page like campaigns earned "+fmt(allTotals.pageLikes)+" new likes at "+fR(likeSpend/allTotals.pageLikes)+" cost per like.");return p.join(" ");})()}</Insight>
              </div>;
            })()}
          </div>
    ...
    with open("/workspaces/media-on-gas/dashboard/src/App.jsx", "w") as f:
        f.write(c)
    print("Done - objective section rebuilt with grouped tables and summary blocks")
