    ...
    new_obj = """{/* OBJECTIVE ASSESSMENT */}
          <div style={{background:P.glass,borderRadius:18,padding:"6px 24px 24px",marginBottom:28,border:"1px solid "+P.rule}}>
            ...
            {(function(){
              ...
              var allTotals={spend:rows.reduce(function(a,r){return a+r.spend;},0),clicks:rows.reduce(function(a,r){return a+r.clicks;},0)};
              var leadRows=rows.filter(function(r){return r.objective==="Leads";});
              var followRows=rows.filter(function(r){return r.objective==="Follows";});
              var likeRows=rows.filter(function(r){return r.objective==="Page Likes";});
              var appRows=rows.filter(function(r){return r.objective==="App Store Clicks";});
              var lpRows=rows.filter(function(r){return r.objective==="Landing Page Clicks"||r.objective==="Traffic";});
              var tLeads=leadRows.reduce(function(a,r){return a+r.result;},0);
              var tFollows=followRows.reduce(function(a,r){return a+r.result;},0);
              var tLikes=likeRows.reduce(function(a,r){return a+r.result;},0);
              var tApp=appRows.reduce(function(a,r){return a+r.result;},0);
              var tLp=lpRows.reduce(function(a,r){return a+r.result;},0);
              var sLeads=leadRows.reduce(function(a,r){return a+r.spend;},0);
              var sFollows=followRows.reduce(function(a,r){return a+r.spend;},0);
              var sLikes=likeRows.reduce(function(a,r){return a+r.spend;},0);
              var sApp=appRows.reduce(function(a,r){return a+r.spend;},0);
              var sLp=lpRows.reduce(function(a,r){return a+r.spend;},0);
              ...
              return <div>
                ...
                <Insight title="Objective Performance" accent={P.rose} icon={Ic.target(P.rose,16)}>{(function(){var p=[];p.push("The selected campaigns invested "+fR(allTotals.spend)+" generating "+fmt(allTotals.clicks)+" total clicks across all objectives.");if(tApp>0)p.push("App Store click campaigns delivered "+fmt(tApp)+" clicks at "+fR(sApp/tApp)+" CPC.");if(tLp>0)p.push("Landing page campaigns drove "+fmt(tLp)+" clicks at "+fR(sLp/tLp)+" CPC.");if(tLeads>0){var lConv=leadRows.reduce(function(a,r){return a+r.clicks;},0);p.push("Lead generation produced "+fmt(tLeads)+" leads at "+fR(sLeads/tLeads)+" CPL"+(lConv>0?", a "+(tLeads/lConv*100).toFixed(1)+"% conversion rate.":"."));}if(tFollows>0)p.push("Follower campaigns acquired "+fmt(tFollows)+" new followers at "+fR(sFollows/tFollows)+" CPF.");if(tLikes>0)p.push("Page like campaigns earned "+fmt(tLikes)+" new likes at "+fR(sLikes/tLikes)+" cost per like.");return p.join(" ");})()}</Insight>
              </div>;
            })()}
          </div>
