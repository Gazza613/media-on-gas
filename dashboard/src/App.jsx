              ...
              {(function(){
                ...
                var audienceRead=(function(){
                  ...
                  if(convAge&&convGen&&convSegShare>0){
                    lines.push("The "+convAge+" "+convGen+" audience is emerging as the highest-propensity segment, capturing "+convSegShare.toFixed(2)+"% of conversions on its own, the strongest economic return per impression in the mix.");
                  } else if(convAge){
                  ...
                  if(convProv&&convProvShare>0){
                    var geoLine=convProv+" is producing a disproportionate "+convProvShare.toFixed(2)+"% share of conversions";
        ...
        {tab==="overview"&&(<div>
          ...
          <div style={{background:P.glass,borderRadius:18,padding:"6px 24px 24px",marginBottom:28,border:"1px solid "+P.rule}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:16}}>
              ...
              <div style={{background:"rgba(0,0,0,0.2)",borderRadius:12,padding:20}}><div style={{fontSize:10,fontWeight:800,color:"rgba(255,255,255,0.5)",letterSpacing:3,fontFamily:fm,textTransform:"uppercase",marginBottom:14}}>Spend Allocation</div><ResponsiveContainer width="100%" height={200}><PieChart><Pie data={[{name:"Facebook",value:computed.fb.spend},{name:"Instagram",value:computed.ig.spend},{name:"TikTok",value:t.spend},{name:"Google",value:computed.gd.spend}].filter(function(x){return x.value>0;})} cx="50%" cy="50%" outerRadius={65} innerRadius={40} paddingAngle={4} dataKey="value" stroke="none" label={function(entry){var total=computed.totalSpend;var pct=total>0?(entry.value/total*100).toFixed(2):0;return pct>0?pct+"%":"";}} labelStyle={{fontSize:7,fontFamily:fm,fill:"rgba(255,255,255,0.9)"}}><Cell fill={P.fb}/><Cell fill={P.ig}/><Cell fill={P.tt}/><Cell fill={P.gd}/></Pie><Tooltip content={<Tip/>} wrapperStyle={{outline:"none"}} cursor={{fill:"rgba(255,255,255,0.05)"}}/></PieChart></ResponsiveContainer><div style={{display:"flex",justifyContent:"center",gap:12,marginTop:8}}>{(function(){var total=computed.totalSpend;return [{n:"FB",v:computed.fb.spend,c:P.fb},{n:"IG",v:computed.ig.spend,c:P.ig},{n:"TT",v:t.spend,c:P.tt},{n:"GD",v:computed.gd.spend,c:P.gd}].map(function(p){var pct=total>0?(p.v/total*100).toFixed(2):"0.00";return <div key={p.n} style={{display:"flex",alignItems:"center",gap:4}}><span style={{width:8,height:8,borderRadius:"50%",background:p.c}}></span><span style={{fontSize:10,color:"rgba(255,255,255,0.7)",fontFamily:fm}}>{p.n} {fR(p.v)} ({pct}%)</span></div>;});})()}</div></div>
            </div>
