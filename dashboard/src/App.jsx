...
// Glass hover effect moved off React state into pure CSS :hover (see
// the gas-hover-card / gas-hover-card-line rules in the global <style>
// block). Previous behaviour fired useState + onMouseEnter/Leave on
// every hover, which on Summary (~30+ hv={true} tiles) made scrolling
// jitter as the mouse passed over each tile in turn — every cross-over
// re-rendered the Glass and its children. CSS :hover does the same
...
function TargetingPersonaCard(props){
  ...
  // Hover lift moved off React state into pure CSS .gas-glow-card:hover —
  // previous onMouseEnter / onMouseLeave + useState re-rendered this
  // component on every mouse pass during scroll, the same root cause as
  // the Glass scroll-jitter bug. CSS custom properties carry the dynamic
  // accent into the hover rule.
  var delay=typeof props.delay==="number"?props.delay:0;
  return <div className="gas-glow-card" style={{position:"relative",background:"linear-gradient(165deg,"+c+"14 0%,"+c+"05 50%,transparent 100%),#0d0520",borderRadius:18,border:"1px solid "+c+"40",padding:"22px 22px 18px",boxShadow:"0 10px 36px rgba(0,0,0,0.35),0 0 60px "+c+"10 inset",display:"flex",flexDirection:"column","--card-accent-border":c+"80","--card-accent-shadow":"0 14px 44px rgba(0,0,0,0.45),0 0 80px "+c+"35,0 0 120px "+c+"22 inset"}}>
...
function GoogleIntentCard(props){
  ...
  // Hover lift via CSS :hover (.gas-glow-card) — same fix as
  // TargetingPersonaCard above. No React re-renders on mouse pass.
  var delay=typeof props.delay==="number"?props.delay:0;
  return <div className="gas-glow-card" style={{position:"relative",background:"linear-gradient(165deg,"+c+"14 0%,"+c+"05 50%,transparent 100%),#0d1a12",borderRadius:18,border:"1px solid "+c+"40",padding:"22px 22px 18px",boxShadow:"0 10px 36px rgba(0,0,0,0.35),0 0 60px "+c+"10 inset",display:"flex",flexDirection:"column","--card-accent-border":c+"80","--card-accent-shadow":"0 14px 44px rgba(0,0,0,0.45),0 0 80px "+c+"35,0 0 120px "+c+"22 inset"}}>
    <div aria-hidden style={{position:"absolute",inset:-1,borderRadius:19,boxShadow:"0 0 28px "+c+"55,0 0 64px "+c+"30",pointerEvents:"none",animation:"personaGlowPulse 4.5s ease-in-out infinite",animationDelay:delay+"s",zIndex:0,willChange:"opacity",transform:"translateZ(0)"}}/>
    <div style={{position:"relative",zIndex:1,display:"flex",flexDirection:"column",height:"100%"}}>
    <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:16,paddingBottom:12,borderBottom:"1px solid "+c+"28"}}>
          ...
          return <div style={{marginTop:14,borderTop:"1px solid "+P.solar+"25",paddingTop:12}}>
            ...
            {L.entries.length>0&&(function(){
              ...
              return <div>
                ...
                <div style={{fontSize:9,fontWeight:800,color:P.label,letterSpacing:1.5,fontFamily:fm,textTransform:"uppercase",margin:"4px 0 6px"}}>All reports sent ({sorted.length}{sorted.length>500?" — newest 500 shown":""})</div>
                <div style={{overflowX:"auto",overflowY:"auto",maxHeight:"50vh",minHeight:200,border:"1px solid "+P.rule,borderRadius:8,overscrollBehavior:"contain",WebkitOverflowScrolling:"touch"}}>
                  <table style={{width:"100%",borderCollapse:"collapse"}}>
                    <thead style={{position:"sticky",top:0,zIndex:1,background:"#0a0418"}}><tr><th style={rowHd}>Sent</th><th style={rowHd}>Client</th><th style={rowHd}>By</th><th style={rowHd}>To</th><th style={rowHd}>Period</th></tr></thead>
                    <tbody>{sorted.slice(0,500).map(function(e,i){var to=[].concat(e.to||[]).join(", ");return <tr key={i}>
...
function ChatPanel(props){
  ...
  {!isOpen&&<div style={{position:"fixed",right:22,bottom:22,zIndex:900,display:"flex",alignItems:"center",gap:10,flexDirection:"row-reverse",pointerEvents:"none",animation:"fabEntrance 0.8s cubic-bezier(0.34,1.56,0.64,1) both"}}>
    {/* AI Expert launcher: 64x64 gradient circle with stacked AI / EXPERT
        text. Two CSS-only halo rings on .gas-fab-target ::before / ::after
        breathe out of phase to draw subtle attention with zero React work.
        Sizing is explicit (width/height/min-w/min-h/aspect-ratio + flex:0
        0 64px) so the flex parent can never squash it into an oval. All
        button styling moved into .gas-fab-btn rules in the global style
        block (was previously inline, which let browser-default button
        padding slip through and stretched the box horizontally). */}
    <div className="gas-fab-target">
      <button className="gas-fab-btn" onClick={openPanel} aria-label="Chat with GAS Media Expert" style={{fontFamily:fm}}>
        <span className="gas-fab-btn-ai">AI</span>
        <span className="gas-fab-btn-expert">Expert</span>
      </button>
    </div>
          ...
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            ...
            <div title="Live data, refreshed from Meta / TikTok / Google every time you load or refresh this page" style={{display:"inline-flex",alignItems:"center",gap:5,padding:"3px 8px 3px 7px",borderRadius:999,border:"1px solid rgba(74,222,128,0.30)",background:"rgba(74,222,128,0.05)",fontFamily:fm,animation:"liveChipBreath 2.4s ease-in-out infinite"}}>
              <span style={{position:"relative",display:"inline-flex",alignItems:"center",justifyContent:"center",width:7,height:7}}>
                <span style={{width:7,height:7,borderRadius:"50%",background:"#4ade80",boxShadow:"0 0 8px rgba(74,222,128,0.85), 0 0 14px rgba(74,222,128,0.45)",animation:"liveDotPulse 2.4s ease-in-out infinite, liveDotGlow 2.4s ease-in-out infinite"}}/>
              </span>
              ...
              <div style={{background:P.glass,borderRadius:18,padding:"6px 28px 28px",marginBottom:28,border:"1px solid "+P.rule}}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:16}}>
                  ...
                  <Reveal minHeight={300} delay={240}><div style={{height:300}}>
                    ...
                    <ChartReveal><ResponsiveContainer width="100%" height="90%">
                      <BarChart data={cpmData.slice().sort(function(a,b){return b.cpm-a.cpm;})} barSize={44} margin={{top:34,right:12,left:0,bottom:0}}><CartesianGrid strokeDasharray="3 3" stroke={P.rule}/><XAxis dataKey="name" tick={{fontSize:11,fill:P.label,fontFamily:fm}} axisLine={false} tickLine={false}/><YAxis tick={{fontSize:10,fill:P.caption,fontFamily:fm}} axisLine={false} tickLine={false} tickFormatter={function(v){return "R"+Number(v).toFixed(2);}}/><Tooltip content={<Tip/>} wrapperStyle={{outline:"none"}} cursor={{fill:"rgba(255,255,255,0.05)"}}/><Legend verticalAlign="bottom" iconType="circle" wrapperStyle={legStyle}/><Bar dataKey="cpm" name="CPM" radius={[6,6,0,0]} fill="rgba(255,255,255,0.55)">{cpmData.slice().sort(function(a,b){return b.cpm-a.cpm;}).map(function(e,i){return <Cell key={i} fill={e.color}/>;})}<LabelList dataKey="cpm" position="top" content={function(lp){var x=lp.x,y=lp.y,width=lp.width,value=lp.value;if(typeof value!=="number"||typeof x!=="number")return null;var cx=x+width/2;var perAd="R"+(value/1000).toFixed(3);return <g><text x={cx} y={y-18} textAnchor="middle" fontSize={11} fontWeight={800} fill="#FFFFFF" fontFamily={fm}>{"R"+Number(value).toFixed(2)}</text><text x={cx} y={y-5} textAnchor="middle" fontSize={8.5} fontStyle="italic" fill="rgba(255,255,255,0.55)" fontFamily={fm}>{"= "+perAd+"/ad"}</text></g>;}}/></Bar></BarChart>
                    </ResponsiveContainer></ChartReveal>
                  </div></Reveal>
                </div>
                  ...
                  {rows.map(function(r,idx){
                    ...
                    return <div key={r.key} style={{display:"flex",alignItems:"center",gap:14,marginBottom:12}}>
                      ...
                      <div style={{flex:1,height:20,background:"rgba(0,0,0,0.4)",borderRadius:10,overflow:"hidden",border:"1px solid "+P.rule,position:"relative"}}>
                        <GrowBar pct={pct} delay={idx*60} style={{height:"100%",background:"linear-gradient(90deg,"+r.color+"cc,"+r.color+"ff)",borderRadius:10,color:r.color}}/>
                      </div>
