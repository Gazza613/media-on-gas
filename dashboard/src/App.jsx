...
function ChatPanel(props){
  ...
  {!isOpen&&<div style={{position:"fixed",right:22,bottom:22,zIndex:900,display:"flex",alignItems:"center",gap:10,flexDirection:"row-reverse",pointerEvents:"none",animation:"fabEntrance 0.8s cubic-bezier(0.34,1.56,0.64,1) both"}}>
    {/* AI Expert launcher: gradient circle with stacked AI / EXPERT text
        in white. Two CSS-only halo rings (::before / ::after on
        .gas-fab-target) breathe out of phase to draw subtle attention
        without any React work, the rings are pure opacity+transform so
        they composite on the GPU and do not steal scroll budget. */}
    <div className="gas-fab-target">
      <button className="gas-fab-btn" onClick={openPanel} aria-label="Chat with GAS Media Expert" style={{position:"relative",width:"100%",height:"100%",borderRadius:"50%",border:"none",cursor:"pointer",background:"linear-gradient(135deg,#FF3D00 0%,#FF6B00 40%,#F96203 70%,#A855F7 130%)",backgroundSize:"200% 200%",animation:"fabShimmer 5s ease-in-out infinite alternate",color:"#fff",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:1,outline:"none",fontFamily:fm,textTransform:"uppercase"}}>
        <span style={{fontSize:15,fontWeight:900,letterSpacing:1.5,lineHeight:1}}>AI</span>
        <span style={{fontSize:7.5,fontWeight:800,letterSpacing:1.8,lineHeight:1,opacity:0.95}}>Expert</span>
      </button>
    </div>
