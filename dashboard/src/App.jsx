...
function Tip(props){if(!props.active||!props.payload||!props.payload.length)return null;return(<div style={{background:"#121212",border:"1px solid rgba(255,255,255,0.2)",borderRadius:12,padding:"12px 16px",boxShadow:"0 8px 32px rgba(0,0,0,0.6)"}}><div style={{fontSize:11,fontWeight:800,color:P.txt,fontFamily:fm,marginBottom:4}}>{props.label}</div>{props.payload.map(function(p,i){var v=p.value;var display="";var n=(p.name||"").toLowerCase();var dn=(p.dataKey||"").toLowerCase();if(dn==="ctr"){display=typeof v==="number"?v.toFixed(2)+"%":v;} else if(n.indexOf("spend")>=0||n.indexOf("cpc")>=0||n.indexOf("cpm")>=0||n.indexOf("cpl")>=0||n.indexOf("cpf")>=0||n.indexOf("cost")>=0||dn.indexOf("value")>=0||n.indexOf("facebook")>=0||n.indexOf("instagram")>=0||n.indexOf("tiktok")>=0||n.indexOf("google")>=0){display="R"+(typeof v==="number"?v.toLocaleString("en-ZA",{minimumFractionDigits:2,maximumFractionDigits:2}):v);}else{display=typeof v==="number"?v.toLocaleString():v;}return<div key={i} style={{fontSize:11,color:p.color||P.sub,fontFamily:fm,lineHeight:1.8}}>{p.name}: {display}</div>;})}</div>);}
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginTop:20}}>
                    <div style={{height:200}}>
                      ...
                      <ResponsiveContainer width="100%" height="85%">
                        <PieChart><Pie data={spendData} dataKey="value" cx="50%" cy="50%" innerRadius={40} outerRadius={68} paddingAngle={3} stroke="none" label={function(e){var x=e.x;var y=e.y;var anchor=e.textAnchor;return<text x={x} y={y} textAnchor={anchor} dominantBaseline="central" style={{fontSize:9,fontFamily:fm,fontWeight:700,fill:e.payload.color||P.txt}}>{e.name+" "+Math.round(e.value/computed.totalSpend*100)+"%"}</text>;}} labelLine={false}>{spendData.map(function(e,i){return <Cell key={i} fill={e.color}/>;})}</Pie><Tooltip content={<Tip/>} wrapperStyle={{outline:"none"}}/></PieChart>
                      </ResponsiveContainer>
                    </div>
                    <div style={{height:200}}>
                      ...
                      <ResponsiveContainer width="100%" height="85%">
                        <BarChart data={impData} barSize={28}><CartesianGrid strokeDasharray="3 3" stroke={P.rule}/><XAxis dataKey="name" tick={{fontSize:10,fill:P.sub,fontFamily:fm}} axisLine={false} tickLine={false}/><YAxis tick={{fontSize:9,fill:P.dim,fontFamily:fm}} axisLine={false} tickLine={false} tickFormatter={function(v){return fmt(v);}}/><Tooltip content={<Tip/>} wrapperStyle={{outline:"none"}} cursor={{fill:"rgba(255,255,255,0.05)"}}/><Bar dataKey="value" name="Impressions" radius={[6,6,0,0]}>{impData.map(function(e,i){return <Cell key={i} fill={e.color}/>;})}</Bar></BarChart>
                      </ResponsiveContainer>
                    </div>
                  </div>
              ...
              {selAdsets2.length>0&&(function(){
                ...
                return <div style={{background:P.glass,borderRadius:18,padding:"6px 28px 28px",marginBottom:28,border:"1px solid "+P.rule}}>
                  ...
                  {topChart.length>=2&&<div style={{height:220,marginBottom:16}}>
                    <div style={{fontSize:9,fontWeight:800,color:P.sub,fontFamily:fm,letterSpacing:2,marginBottom:8,textAlign:"center"}}>TOP AD SET EFFICIENCY (RESULTS)</div>
                    <ResponsiveContainer width="100%" height="85%">
                  ...
                  <Insight title="Targeting Read" accent={P.solar} icon={Ic.radar(P.solar,14)}>{(function(){var lines=[];if(topAd.length>0){var best=topAd[0];lines.push("Strongest ad set: \""+best.name+"\" on "+best.platform+" \u2014 "+fmt(best.result)+" results at "+fR(best.costPer)+"/result with "+best.ctr.toFixed(2)+"% CTR. "+(topAd.length>1?"The top "+Math.min(topAd.length,4)+" ad sets are delivering above-benchmark efficiency and warrant increased budget allocation.":""));}if(worstAd.length>0){var worst=worstAd[0];lines.push((worstAd.length===1?"One ad set requires":""+worstAd.length+" ad sets require")+" attention. \""+worst.name+"\" has consumed "+fR(worst.spend)+" with "+(worst.result===0?"no attributable results":worst.ctr.toFixed(2)+"% CTR")+". "+(worstAd.length>1?"Recommend pausing underperformers and reallocating budget to the winning ad sets listed above.":"Review targeting parameters, creative alignment, and conversion tracking."));}return lines.join(" ")||"Insufficient ad set data for targeting analysis.";})()}</Insight>
                </div>;
              })()}
              ...
              {grandT2>0&&<div style={{background:P.glass,borderRadius:18,padding:"6px 28px 28px",marginBottom:28,border:"1px solid "+P.rule}}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:16}}>
                  <div style={{height:220}}>
                    ...
                    <ResponsiveContainer width="100%" height="85%">
                      <BarChart data={communityData} barSize={36}><CartesianGrid strokeDasharray="3 3" stroke={P.rule}/><XAxis dataKey="name" tick={{fontSize:10,fill:P.sub,fontFamily:fm}} axisLine={false} tickLine={false}/><YAxis tick={{fontSize:9,fill:P.dim,fontFamily:fm}} axisLine={false} tickLine={false} tickFormatter={function(v){return fmt(v);}}/><Tooltip content={<Tip/>} wrapperStyle={{outline:"none"}} cursor={{fill:"rgba(255,255,255,0.05)"}}/><Bar dataKey="total" name="Total Followers" radius={[6,6,0,0]}>{communityData.map(function(e,i){return <Cell key={i} fill={e.color}/>;})}</Bar></BarChart>
                    </ResponsiveContainer>
                  </div>
                  <div style={{height:220}}>
                    ...
                    <ResponsiveContainer width="100%" height="85%">
                      <BarChart data={communityData.filter(function(c){return c.earned>0;})} barSize={36}><CartesianGrid strokeDasharray="3 3" stroke={P.rule}/><XAxis dataKey="name" tick={{fontSize:10,fill:P.sub,fontFamily:fm}} axisLine={false} tickLine={false}/><YAxis tick={{fontSize:9,fill:P.dim,fontFamily:fm}} axisLine={false} tickLine={false} tickFormatter={function(v){return fmt(v);}}/><Tooltip content={<Tip/>} wrapperStyle={{outline:"none"}} cursor={{fill:"rgba(255,255,255,0.05)"}}/><Bar dataKey="earned" name="Earned Followers" radius={[6,6,0,0]}>{communityData.filter(function(c){return c.earned>0;}).map(function(e,i){return <Cell key={i} fill={e.color}/>;})}</Bar></BarChart>
                    </ResponsiveContainer>
                  </div>
                </div>
          ...
          <div style={{background:P.glass,borderRadius:18,padding:"6px 24px 24px",marginBottom:28,border:"1px solid "+P.rule}}>
            ...
            {(function(){
              ...
              return <div>
                {sections}
                <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:16}}>
                  <Glass accent={P.fb} hv={true} st={{padding:16,textAlign:"center"}}><div style={{fontSize:8,color:"rgba(255,255,255,0.55)",fontFamily:fm,letterSpacing:2,marginBottom:4}}>CLICKS TO APP INSTALL</div><div style={{fontSize:22,fontWeight:900,color:P.fb,fontFamily:fm}}>{fmt(tApp)}</div><div style={{fontSize:9,color:"rgba(255,255,255,0.5)",fontFamily:fm,marginTop:4}}>CPC: {fR(tApp>0?sApp/tApp:0)}</div></Glass>
                  <Glass accent={P.cyan} hv={true} st={{padding:16,textAlign:"center"}}><div style={{fontSize:8,color:"rgba(255,255,255,0.55)",fontFamily:fm,letterSpacing:2,marginBottom:4}}>CLICKS TO LANDING PAGE</div><div style={{fontSize:22,fontWeight:900,color:P.cyan,fontFamily:fm}}>{fmt(tLp)}</div><div style={{fontSize:9,color:"rgba(255,255,255,0.5)",fontFamily:fm,marginTop:4}}>CPC: {fR(tLp>0?sLp/tLp:0)}</div></Glass>
                  <Glass accent={P.rose} hv={true} st={{padding:16,textAlign:"center"}}><div style={{fontSize:8,color:"rgba(255,255,255,0.55)",fontFamily:fm,letterSpacing:2,marginBottom:4}}>LEADS</div><div style={{fontSize:22,fontWeight:900,color:P.rose,fontFamily:fm}}>{fmt(tLeads)}</div><div style={{fontSize:9,color:"rgba(255,255,255,0.5)",fontFamily:fm,marginTop:4}}>CPL: {fR(tLeads>0?sLeads/tLeads:0)}</div></Glass>
                  <Glass accent={P.tt} hv={true} st={{padding:16,textAlign:"center"}}><div style={{fontSize:8,color:"rgba(255,255,255,0.55)",fontFamily:fm,letterSpacing:2,marginBottom:4}}>FOLLOWERS & LIKES</div><div style={{fontSize:22,fontWeight:900,color:P.mint,fontFamily:fm}}>{fmt(tFollows)}</div><div style={{fontSize:9,color:"rgba(255,255,255,0.5)",fontFamily:fm,marginTop:4}}>CPF: {fR(tFollows>0?sFollows/tFollows:0)}</div></Glass>
          ...
          <div style={{background:P.glass,borderRadius:18,padding:"6px 24px 24px",marginBottom:28,border:"1px solid "+P.rule}}>
            ...
            {(function(){
              ...
              return <div>
                ...
                <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:20}}>
                  <Glass accent={P.mint} hv={true} st={{padding:16,textAlign:"center"}}><div style={{fontSize:8,color:"rgba(255,255,255,0.55)",fontFamily:fm,letterSpacing:2,marginBottom:4}}>TOTAL COMMUNITY</div><div style={{fontSize:22,fontWeight:900,color:P.mint,fontFamily:fm}}>{fmt(grandTotal)}</div></Glass>
                  <Glass accent={P.ember} hv={true} st={{padding:16,textAlign:"center"}}><div style={{fontSize:8,color:"rgba(255,255,255,0.55)",fontFamily:fm,letterSpacing:2,marginBottom:4}}>EARNED THIS PERIOD</div><div style={{fontSize:22,fontWeight:900,color:P.ember,fontFamily:fm}}>+{fmt(totalEarned)}</div></Glass>
                  <Glass accent={P.orchid} hv={true} st={{padding:16,textAlign:"center"}}><div style={{fontSize:8,color:"rgba(255,255,255,0.55)",fontFamily:fm,letterSpacing:2,marginBottom:4}}>GROWTH RATE</div><div style={{fontSize:22,fontWeight:900,color:P.orchid,fontFamily:fm}}>{grandTotal>0?(totalEarned/grandTotal*100).toFixed(1)+"%":"\u2014"}</div></Glass>
                  <Glass accent={P.solar} hv={true} st={{padding:16,textAlign:"center"}}><div style={{fontSize:8,color:"rgba(255,255,255,0.55)",fontFamily:fm,letterSpacing:2,marginBottom:4}}>COST PER MEMBER</div><div style={{fontSize:22,fontWeight:900,color:P.solar,fontFamily:fm}}>{totalEarned>0?fR(totalSpend/totalEarned):"\u2014"}</div></Glass>
                </div>
