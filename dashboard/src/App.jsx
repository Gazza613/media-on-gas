  ...
  return(<div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"radial-gradient(ellipse at 50% 20%,#1a0b2e 0%,#0d0618 45%,#06020e 100%)",fontFamily:ff,position:"relative",overflow:"hidden"}}>
    ...
    <div style={{width:"100%",maxWidth:380,padding:32,position:"relative",zIndex:2,animation:"gasEnter 0.8s cubic-bezier(0.2,0.8,0.2,1) both"}}>
      ...
      <div style={{background:"rgba(30,18,50,0.5)",borderRadius:16,padding:28,backdropFilter:"blur(24px)",WebkitBackdropFilter:"blur(24px)",border:"1px solid rgba(249,98,3,0.25)",animation:"gasCardBreather 4.5s ease-in-out infinite, gasEnter 0.9s cubic-bezier(0.2,0.8,0.2,1) 0.25s both"}}>
        ...
        <button onClick={handleLogin} disabled={busy} onMouseEnter={function(e){if(!busy){e.currentTarget.style.transform="translateY(-2px) scale(1.015)";e.currentTarget.style.animation="none";e.currentTarget.style.boxShadow="0 14px 36px rgba(249,98,3,0.58),0 0 52px rgba(249,98,3,0.45),0 0 96px rgba(168,85,247,0.28),inset 0 0 0 1px rgba(255,255,255,0.3)";}}} onMouseLeave={function(e){e.currentTarget.style.transform="translateY(0) scale(1)";e.currentTarget.style.animation="gasBtnAura 3s ease-in-out infinite";e.currentTarget.style.boxShadow="";}} style={{width:"100%",background:busy?"#555":"linear-gradient(135deg,#FF3D00 0%,#FF6B00 45%,#F96203 100%)",border:"none",borderRadius:12,padding:"15px 24px",color:"#fff",fontSize:13,fontWeight:900,fontFamily:fm,cursor:busy?"wait":"pointer",letterSpacing:2.5,opacity:busy?0.7:1,transition:"transform 0.22s cubic-bezier(0.2,0.8,0.2,1), box-shadow 0.3s",position:"relative",overflow:"hidden",animation:busy?"none":"gasBtnAura 3s ease-in-out infinite"}}>
          ...
          {/* sheen that sweeps across every 3 seconds */}
          {!busy&&<span style={{position:"absolute",top:0,left:0,width:"28%",height:"100%",background:"linear-gradient(90deg,transparent,rgba(255,255,255,0.45) 50%,transparent)",animation:"gasBtnSheenFast 3s ease-in-out infinite",pointerEvents:"none"}}/>}
        </button>
