            ...
            return <div>
              ...
              <div style={{background:P.glass,borderRadius:18,padding:"6px 28px 28px",marginBottom:28,border:"1px solid "+P.rule}}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:16}}>
                  ...
                  <div style={{height:300}}>
                    ...
                    <ResponsiveContainer width="100%" height="90%">
                      <BarChart data={cpmData.slice().sort(function(a,b){return b.cpm-a.cpm;})} barSize={44} margin={{top:24,right:12,left:0,bottom:0}}><CartesianGrid strokeDasharray="3 3" stroke={P.rule}/><XAxis dataKey="name" tick={{fontSize:11,fill:P.sub,fontFamily:fm}} axisLine={false} tickLine={false}/><YAxis tick={{fontSize:10,fill:P.dim,fontFamily:fm}} axisLine={false} tickLine={false} tickFormatter={function(v){return"R"+v;}}/><Tooltip content={<Tip/>} wrapperStyle={{outline:"none"}} cursor={{fill:"rgba(255,255,255,0.05)"}}/><Legend verticalAlign="bottom" iconType="circle" wrapperStyle={legStyle}/><Bar dataKey="cpm" name="CPM" radius={[6,6,0,0]} fill="rgba(255,255,255,0.55)">{cpmData.slice().sort(function(a,b){return b.cpm-a.cpm;}).map(function(e,i){return <Cell key={i} fill={e.color}/>;})}<LabelList dataKey="cpm" position="top" formatter={function(v){return"R"+v;}} style={lblStyle}/></Bar></BarChart>
                    </ResponsiveContainer>
                  </div>
                </div>
              ...
              <div style={{background:P.glass,borderRadius:18,padding:"6px 28px 28px",marginBottom:28,border:"1px solid "+P.rule}}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,marginBottom:20}}>
                  ...
                  <Glass accent={P.solar} hv={true} st={{padding:18,textAlign:"center"}}><div style={{fontSize:10,color:"rgba(255,255,255,0.55)",fontFamily:fm,letterSpacing:2,marginBottom:6}}>BLENDED CLICK THROUGH RATE %</div><div style={{fontSize:28,fontWeight:900,color:blCtr>2?P.mint:blCtr>1?P.txt:P.warning,fontFamily:fm,lineHeight:1}}>{blCtr.toFixed(2)+"%"}</div><div style={{fontSize:9,color:P.sub,fontFamily:fm,marginTop:8}}>{"SA benchmark: 0.9\u20131.4%"}</div></Glass>
                  ...
                  <div style={{height:300}}>
                    ...
                    <ResponsiveContainer width="100%" height="90%">
                      <ComposedChart data={cpcData.map(function(d){var pl=Object.keys(platBreak).filter(function(k){return(platShort[k]||k)===d.name;})[0];return{name:d.name,cpc:d.cpc,clicks:pl?platBreak[pl].clicks:0,color:d.color};}).sort(function(a,b){return b.cpc-a.cpc;})} barSize={38} margin={{top:24,right:16,left:0,bottom:0}}>
                        ...
                        <Tooltip content={<Tip/>} wrapperStyle={{outline:"none"}} cursor={{fill:"rgba(255,255,255,0.05)"}}/>
                        <Legend verticalAlign="bottom" iconType="circle" wrapperStyle={legStyle}/>
                        <Bar yAxisId="left" dataKey="cpc" name="CPC" radius={[6,6,0,0]} fill="rgba(255,255,255,0.55)">{cpcData.slice().sort(function(a,b){return b.cpc-a.cpc;}).map(function(e,i){return <Cell key={i} fill={e.color}/>;})}<LabelList dataKey="cpc" position="top" formatter={function(v){return"R"+v;}} style={lblStyle}/></Bar>
