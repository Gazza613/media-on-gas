...
function LoginScreen(props){
  ...
  return(<div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"radial-gradient(ellipse at 50% 20%,#1a0b2e 0%,#0d0618 45%,#06020e 100%)",fontFamily:ff,position:"relative",overflow:"hidden"}}>
    ...
    <div style={{width:"100%",maxWidth:380,padding:32,position:"relative",zIndex:2,animation:"gasEnter 0.8s cubic-bezier(0.2,0.8,0.2,1) both"}}>
      ...
      <div style={{position:"relative",animation:"gasCardBreath 7s ease-in-out infinite"}}>
        {/* Slow orbital ring, sits behind the card */}
        <div style={{position:"absolute",inset:-18,borderRadius:24,pointerEvents:"none",opacity:0.45,zIndex:0}}>
          <div style={{position:"absolute",inset:0,borderRadius:24,background:"conic-gradient(from 0deg,transparent 0deg,rgba(249,98,3,0.28) 40deg,transparent 110deg,transparent 200deg,rgba(168,85,247,0.22) 240deg,transparent 310deg,transparent 360deg)",animation:"gasRingSpin 18s linear infinite",filter:"blur(14px)"}}/>
        </div>
      <div style={{position:"relative",zIndex:1,background:"rgba(30,18,50,0.5)",border:"1px solid "+P.rule,borderRadius:16,padding:28,backdropFilter:"blur(24px)",WebkitBackdropFilter:"blur(24px)",animation:"gasBorderGlow 5s ease-in-out infinite, gasEnter 0.9s cubic-bezier(0.2,0.8,0.2,1) 0.25s both"}}>
        ...
        <button onClick={handleLogin} disabled={busy} onMouseEnter={function(e){if(!busy){e.currentTarget.style.transform="translateY(-2px) scale(1.01)";e.currentTarget.style.animation="none";e.currentTarget.style.boxShadow="0 10px 30px rgba(249,98,3,0.5),0 0 40px rgba(249,98,3,0.4),0 0 72px rgba(168,85,247,0.25)";}}} onMouseLeave={function(e){e.currentTarget.style.transform="translateY(0) scale(1)";e.currentTarget.style.animation="gasButtonHalo 3.2s ease-in-out infinite";e.currentTarget.style.boxShadow="";}} style={{width:"100%",background:busy?"#555":gEmber,border:"none",borderRadius:10,padding:"14px 24px",color:"#fff",fontSize:13,fontWeight:800,fontFamily:fm,cursor:busy?"wait":"pointer",letterSpacing:2,opacity:busy?0.7:1,transition:"transform 0.22s cubic-bezier(0.2,0.8,0.2,1), box-shadow 0.3s",position:"relative",overflow:"hidden",animation:busy?"none":"gasButtonHalo 3.2s ease-in-out infinite"}}>
          <span style={{position:"relative",zIndex:2}}>{busy?"AUTHENTICATING...":"SIGN IN"}</span>
          {!busy&&<span style={{position:"absolute",top:0,left:"-50%",width:"50%",height:"100%",background:"linear-gradient(90deg,transparent,rgba(255,255,255,0.28),transparent)",animation:"gasSheen 3.2s ease-in-out infinite",pointerEvents:"none"}}/>}
        </button>
