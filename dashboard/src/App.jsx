    ...
    fetch(props.apiBase+"/api/ads?from="+encodeURIComponent(props.dateFrom)+"&to="+encodeURIComponent(props.dateTo)+"&debug=1",{headers:{"x-session-token":props.session||""}})
      ...
      .then(function(x){
        ...
        hit.forEach(function(a){
          ...
          if(!byCamp[cid])byCamp[cid]={id:cid,name:String(a.campaignName||"(unnamed)"),metaObj:String(a._debugMetaObjective||"?"),pageLikeOpt:a._debugPageLikeOpt===true,optGoals:String(a._debugOptGoals||"?"),spend:0,plats:{}};
          var p=a.platform||"?";
    ...
    <div style={{background:P.cosmos,border:"1px solid "+P.rule,borderRadius:20,padding:"22px 26px",width:1100,maxWidth:"96vw",maxHeight:"calc(100vh - 48px)",display:"flex",flexDirection:"column",boxShadow:"0 24px 80px rgba(0,0,0,0.6)"}}>
      ...
      {view[0]==="audit"&&props.isSuperadmin&&<div style={{marginBottom:14,padding:"12px 14px",background:P.cyan+"08",border:"1px solid "+P.cyan+"30",borderLeft:"3px solid "+P.cyan,borderRadius:"0 12px 12px 0"}}>
        ...
        {dbgState[0].rows&&Object.keys(dbgState[0].rows).length>0&&<div style={{marginTop:10,display:"flex",flexDirection:"column",gap:14,maxHeight:"60vh",overflowY:"auto",overflowX:"hidden",paddingRight:8,WebkitOverflowScrolling:"touch"}}>
          ...
          <div style={{fontSize:9.5,color:P.caption,fontFamily:ff,fontStyle:"italic",lineHeight:1.5}}>Literal Meta actions map, split per campaign then per publisher (no cross-campaign aggregation). Open the single campaign you are reconciling, read its Facebook block, and find the key whose value equals Meta Ads Manager's per-platform "Results". Send me that key — that is the action_type to wire into the Follows & Likes result. If no key matches, Meta is not returning a page-join for that campaign (it is engagement/visit-optimised) and the result must come from whole-account snapshot growth, not per-ad attribution.</div>
        </div>}
      </div>}
      ...
      <div style={{flex:1,minWidth:0}}>
        ...
        {tab==="summary"&&(<div>
          ...
          {/* The per-client KPI objectives used to render here at the top.
              ...
              profiled clients), styled as glass standout boxes. */}
          ...
          {(function(){
            ...
            var fbT2=0;var igT2=0;var igGrowth=0;var fbGrowth=0;var fbGrowthKnown=false;matchedPages3.forEach(function(mp){fbT2+=mp.followers_count||mp.fan_count||0;if(typeof mp.follower_growth==="number"){fbGrowth+=mp.follower_growth;fbGrowthKnown=true;}if(mp.instagram_business_account){igT2+=mp.instagram_business_account.followers_count||0;igGrowth+=mp.instagram_business_account.follower_growth||0;}});
            ...
            return <div>
              ...
              {ecoProfiled?(function(){
                ...
                ordered=ordered.slice(0,8); // 8 max = a clean 2 rows of 4
              ...
              {grandT2>0&&<div style={{background:P.glass,borderRadius:18,padding:"6px 28px 28px",marginBottom:28,border:"1px solid "+P.rule}}>
                ...
                {/* FB page-like campaign panel. UNLIKE Instagram, Meta DOES
                    attribute the page follow per ad for PAGE_LIKES-optimised
                    campaigns (the "Follows or likes" result). c.pageLikes is
                    already gated upstream (only populated when the ad set
                    optimises PAGE_LIKES, validated via optimization_goal),
                    so Step 1 here is a REAL paid follow count, not a proxy.
                    Step 2 is whole-account net page growth from our daily
                    snapshots, "building" until history spans the window. */}
                {(function(){
                  ...
                  return <div style={{marginTop:14,padding:"14px 18px",background:"linear-gradient(135deg,"+P.fb+"10 0%,"+P.fb+"05 60%,transparent)",border:"1px solid "+P.fb+"35",borderLeft:"3px solid "+P.fb,borderRadius:"0 12px 12px 0"}}>
                    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:14,marginBottom:10}}>
                      ...
                      <div><div style={{fontSize:9,color:P.caption,letterSpacing:1.5,textTransform:"uppercase",fontFamily:fm,marginBottom:4}}>Net Page Growth</div><div style={{fontSize:18,fontWeight:900,color:P.fb,fontFamily:fm}}>{fbGrowthKnown?(fbGrowth>0?"+"+fmt(fbGrowth):fbGrowth<0?fmt(fbGrowth):"0"):"-"}</div><div style={{fontSize:9,color:P.caption,fontFamily:fm,marginTop:2}}>{fbGrowthKnown?"whole account, paid + organic":"building snapshot history"}</div></div>
                    </div>
                    <div style={{fontSize:10,color:P.caption,fontFamily:fm,fontStyle:"italic",lineHeight:1.6}}>
                ...
                return <Reveal minHeight={160}><div style={{marginBottom:24}}>
                  ...
                  {fSteps.length>=2&&<div style={{background:"linear-gradient(165deg,"+P.mint+"12 0%,"+P.mint+"04 55%,transparent 100%),#0b0716",borderRadius:16,border:"1px solid "+P.mint+"2E",boxShadow:"0 10px 34px rgba(0,0,0,0.32),0 0 52px "+P.mint+"0E inset",padding:20}}>
                    ...
                    {fSteps.map(function(s,i){var w=Math.max(5,(s.v||0)/fM*100);return <div key={i} style={{marginBottom:14}}>
                      ...
                      {i<fSteps.length-1&&fSteps[i].v>0&&(function(){var cont=Math.min(100,Math.max(0,fSteps[i+1].v/fSteps[i].v*100));return <div style={{fontSize:9.5,color:P.caption,fontFamily:fm,marginTop:5}}>{cont.toFixed(2)}% continue, {(100-cont).toFixed(2)}% drop off</div>;})()}
                    </div>;})}
                  </div>}
                </div></Reveal>;
              })()}
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,margin:"24px 0"}}>
                ...
                <Reveal minHeight={300} delay={80}><div style={{background:"linear-gradient(165deg,"+P.mint+"12 0%,"+P.mint+"04 55%,transparent 100%),#0b0716",borderRadius:16,border:"1px solid "+P.mint+"2E",boxShadow:"0 10px 34px rgba(0,0,0,0.32),0 0 52px "+P.mint+"0E inset",padding:20,height:"100%"}}>
                  ...
                  {funnel.map(function(f,i){var w=Math.max(6,f.v/fMax*100);return <div key={i} style={{marginBottom:16}}>
                    ...
                    {i<funnel.length-1&&funnel[i].v>0&&<div style={{fontSize:9,color:P.caption,fontFamily:fm,marginTop:5,letterSpacing:0.5}}>{Math.min(100,Math.max(0,funnel[i+1].v/funnel[i].v*100)).toFixed(2)}% continue to {funnel[i+1].label.toLowerCase()}</div>}
                  </div>;})}
                </div></Reveal>
              </div>
        ...
        {tab==="overview"&&(<div>
          ...
          {/* OBJECTIVE KEY METRICS. Hidden for a client that has a KPI
              ...
              campaign-derived block classifies awareness spend as
        ...
        {tab==="community"&&(<div>
          ...
          <div style={{background:P.glass,borderRadius:18,padding:"6px 24px 24px",marginBottom:28,border:"1px solid "+P.rule}}>
            ...
            {(function(){
              ...
              var fbTotal=0;var igTotal=0;var fbGrowthKnown=false;
              if(matchedPages2.length>0){
                matchedPages2.forEach(function(mp){fbTotal+=mp.followers_count||mp.fan_count||0;if(typeof mp.follower_growth==="number"){fbEarned+=mp.follower_growth;fbGrowthKnown=true;}if(mp.instagram_business_account){igTotal+=mp.instagram_business_account.followers_count||0;igEarned+=mp.instagram_business_account.follower_growth||0;}});
              ...
              return <div>
                ...
                <Insight title="Community Growth Analysis" accent={P.mint} icon={Ic.users(P.mint,16)}>{(function(){var p=[];if(totalEarned===0&&grandTotal===0){return "No community data available for the selected campaigns.";}if(grandTotal>0){p.push("The brand\'s total social community stands at "+fmt(grandTotal)+" members across Facebook, Instagram, and TikTok.");}if(totalEarned>0){p.push("During the selected period, the community grew by "+fmt(totalEarned)+" new members with "+fR(totalSpend)+" invested at a blended cost of "+fR(totalSpend/totalEarned)+" per new member.");}if(fbTotal>0){p.push("Facebook leads with "+fmt(fbTotal)+" total followers"+(fbGrowthKnown&&fbEarned>0?", growing by "+fmt(fbEarned)+" followers during this period. This figure represents total page growth, organic and paid combined, as Meta does not attribute the follow action directly to paid campaigns":fbGrowthKnown?", with no measurable net page growth in this period":". Net page-follow growth for this exact period is still building: Meta does not attribute page follows to individual ads, so this is read from our own daily whole-account page snapshots, which do not yet cover the full selected range")+". Each new follower permanently increases organic News Feed distribution.");}if(igTotal>0){p.push("Instagram has "+fmt(igTotal)+" total followers"+(igEarned>0?", growing by "+fmt(igEarned)+" followers during this period. This figure represents total profile growth, organic and paid combined, as Meta does not attribute the follow action directly to paid campaigns":"")+". Instagram followers directly increase Stories, Reels, and Feed visibility.");}if(ttEarned>0){p.push("TikTok has "+fmt(ttTotal)+" total followers, growing by "+fmt(ttEarned)+" new follows this period at "+fR(ttSpend/ttEarned)+" cost per follow. Each TikTok follower feeds into the For You page recommendation engine, amplifying organic reach.");}return p.join(" ");})()}</Insight>
              </div>;
            })()}
          </div>
        </div>)}
        ...
        {tab==="optimise"&&!isClient&&(<div>
          ...
          {/* ============================================================
              ...
              Senior-analyst memo on the structural changes the GAS team
              should make to 5x / 10x performance. Computed entirely
              from the selected campaigns + ads in the current window so
              the recommendation tracks the data, not a static template.
              ============================================================ */}
          {sel.length>0&&(function(){
            // ---- Compute the audit ---------------------------------------
            var fmtPct=function(n){return (parseFloat(n)||0).toFixed(2)+"%";};
            var camps2=sel.slice();
            var totalSpend=camps2.reduce(function(s,c){return s+parseFloat(c.spend||0);},0);
            var totalImps=camps2.reduce(function(s,c){return s+parseFloat(c.impressions||0);},0);
            var totalClicks=camps2.reduce(function(s,c){return s+parseFloat(c.clicks||0);},0);
            var blCtr=totalImps>0?(totalClicks/totalImps*100):0;
            var blCpc=totalClicks>0?(totalSpend/totalClicks):0;
            var blCpm=totalImps>0?(totalSpend/totalImps*1000):0;

            // Group by platform + objective so we can talk about
            // structure (fragmentation) and mix (platform/objective spread).
            var byPlat={},byObj={};
            camps2.forEach(function(c){
              var p=c.platform||"Unknown",o=String(c.objective||"unknown").toLowerCase();
              if(!byPlat[p])byPlat[p]={n:0,spend:0};
              byPlat[p].n++;byPlat[p].spend+=parseFloat(c.spend||0);
              if(!byObj[o])byObj[o]={n:0,spend:0,clicks:0,imps:0};
              byObj[o].n++;byObj[o].spend+=parseFloat(c.spend||0);
              byObj[o].clicks+=parseFloat(c.clicks||0);byObj[o].imps+=parseFloat(c.impressions||0);
            });
            var platforms=Object.keys(byPlat);
            var objectives=Object.keys(byObj);

            // Top spender, top performer (best CTR with meaningful imps),
            // worst spend-leak (high spend + low CTR).
            var sortedBySpend=camps2.slice().sort(function(a,b){return parseFloat(b.spend||0)-parseFloat(a.spend||0);});
            var topSpender=sortedBySpend[0];
            var topThreeShare=sortedBySpend.slice(0,3).reduce(function(s,c){return s+parseFloat(c.spend||0);},0);
            var topThreePct=totalSpend>0?(topThreeShare/totalSpend*100):0;

            var realCtr=camps2.filter(function(c){return parseFloat(c.impressions||0)>=5000;});
            var topPerf=realCtr.slice().sort(function(a,b){return parseFloat(b.ctr||0)-parseFloat(a.ctr||0);})[0]||null;
            var topPerfCtr=topPerf?parseFloat(topPerf.ctr||0):0;

            var spendLeakers=camps2.filter(function(c){
              return parseFloat(c.spend||0)>=5000 && parseFloat(c.impressions||0)>=10000 && parseFloat(c.ctr||0)<0.8;
            }).sort(function(a,b){return parseFloat(b.spend||0)-parseFloat(a.spend||0);});
            var leakSpend=spendLeakers.reduce(function(s,c){return s+parseFloat(c.spend||0);},0);

            // Frequency saturation (Meta > 3, TikTok > 6).
            var fatigued=camps2.filter(function(c){
              var freq=parseFloat(c.frequency||0);var p=String(c.platform||"").toLowerCase();
              var limit=p.indexOf("tiktok")>=0?6:3;
              return freq>=limit && parseFloat(c.spend||0)>=2000;
            });

            // Creative format diversity from adsList (uses the same
            // top-ad-per-campaign mapping the flag card already builds).
            var formatSet={};
            ...
            // Scale-ready: high CTR, healthy frequency, results > 0,
            // not pacing behind. Same rule as Command Centre's SCALE.
            var scaleReady=camps2.filter(function(c){
              var ctr=parseFloat(c.ctr||0),freq=parseFloat(c.frequency||0);
              var p=String(c.platform||"").toLowerCase();
              var limit=p.indexOf("tiktok")>=0?6:3;
              return ctr>=1.5 && freq<limit && parseFloat(c.spend||0)>=2000;
            });
            var scaleSpend=scaleReady.reduce(function(s,c){return s+parseFloat(c.spend||0);},0);
            var scalePct=totalSpend>0?(scaleSpend/totalSpend*100):0;

            // Single-platform concentration (algorithm dependency risk).
            ...
            // Funnel coverage — do we have awareness + consideration +
            // conversion, or are we all on one tier?
            ...
            // ---- Synthesise findings -------------------------------------
            var findings=[];
            if(topThreePct>=70&&camps2.length>=5){
              findings.push({title:"Spend is concentrated in 3 campaigns",detail:"The top 3 campaigns hold "+topThreePct.toFixed(2)+"% of total spend. If any one of those decays (creative fatigue, audience saturation, auction shifts), "+Math.round(topThreePct/3)+"% of the budget moves with it. World-class accounts ladder spend across a 5-7 campaign portfolio with planned succession."});
            }
            if(topPerf&&topPerfCtr>=blCtr*1.6&&realCtr.length>=3){
              var multiple=blCtr>0?(topPerfCtr/blCtr):0;
              findings.push({title:"Your best campaign is "+multiple.toFixed(1)+"x the blended CTR",detail:"‘"+topPerf.campaignName+"’ runs at "+fmtPct(topPerfCtr)+" CTR while the blended account sits at "+fmtPct(blCtr)+". The creative + audience pairing in that one campaign is the formula. Most of the upside on this account is replicating that structure across the rest of the portfolio, not spending more on the average."});
            }
            ...
            if(formatCount>0&&formatCount<3&&(adsList||[]).length>=10){
              findings.push({title:"Creative formats are concentrated, only "+formatCount+" in rotation",detail:"Best-in-class accounts ship across 3+ formats (static, carousel, short-form video, UGC) so the algorithm can pick winners by placement. Limited formats cap the ceiling regardless of how much budget you add."});
            }
            if(topPlatPct>=85&&platforms.length>=2){
              findings.push({title:topPlatPct.toFixed(2)+"% of spend lives on "+dominantPlat,detail:"That level of single-platform concentration ties account-wide performance to one algorithm's mood. The accounts that compound performance year-on-year run a deliberate 60/30/10 mix so they always have a B and C option warming when the A option tightens."});
            }
            if(funnelTiers<2&&camps2.length>=3){
            ...
            // ---- 5x play (concrete, this-week actions) -------------------
            var fivex=[];
            ...
            if(topPerf){
              fivex.push("Clone ‘"+topPerf.campaignName+"’ as the template for every same-objective campaign on the account. Same creative format, same hook structure, same audience width. Most of the lift on this account is structural replication, not new creative ideation.");
            }
            if(fatigued.length>0){
            ...
            if(formatCount<3){
              fivex.push("Add the missing formats to the rotation. If you're heavy on static, ship a 9:16 video variant. If video-heavy, add a carousel test. Format diversity unlocks 20-30% more efficient inventory.");
            }
            if(fivex.length===0){
            ...
            // ---- 10x play (the strategic moonshot) -----------------------
            var tenx=[];
            ...
            tenx.push("Consolidate to "+Math.max(3,Math.min(7,Math.round(camps2.length*0.6)))+" CBO campaigns per objective on Meta. Ad-set fragmentation is the single biggest tax on the algorithm. Fewer, bigger campaigns let Meta's learning phase complete and CPMs compress 15-25%.");
            tenx.push("Adopt a 90-day creative cadence. 3-5 fresh creative variants in market every fortnight, with the previous fortnight's winners scaled and losers cut. Top-1% accounts ship 12+ creatives per quarter per platform.");
            if(platforms.length<3){
              tenx.push("Add the third platform. Whatever's missing (TikTok if Meta-heavy, Meta if TikTok-heavy, Google Search if neither) closes the audience leakage. The lift comes from incremental reach, not stealing share from the existing platforms.");
            }
            tenx.push("Build first-party data as the moat. Pixel + Conversions API + offline conversion uploads turn the dashboard from a reporting layer into an attribution engine. Once you can see real CAC by campaign by audience, the budget decisions become arithmetic, not opinion.");

            // ---- Render --------------------------------------------------
            return <div style={{marginTop:28,padding:"28px 30px",borderRadius:18,background:"linear-gradient(135deg,#0a0418 0%,#100624 50%,#1a0a30 100%)",border:"1px solid "+P.ember+"35",position:"relative",overflow:"hidden"}}>
              <div style={{position:"absolute",top:-50,right:-50,width:200,height:200,background:"radial-gradient(circle,"+P.ember+"15 0%,transparent 70%)",pointerEvents:"none"}}></div>
              <div style={{position:"relative",zIndex:1}}>
                <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:6}}>
                  <div style={{width:40,height:40,borderRadius:10,background:"linear-gradient(135deg,"+P.ember+"40,"+P.blaze+"40)",border:"1px solid "+P.ember+"66",display:"flex",alignItems:"center",justifyContent:"center"}}>{Ic.crown(P.ember,20)}</div>
                  <div>
                    <div style={{fontSize:11,fontWeight:900,color:P.ember,fontFamily:fm,letterSpacing:3,textTransform:"uppercase"}}>Head Data Analyst · Recommendation</div>
                ...
                <div style={{fontSize:10,color:P.caption,fontFamily:fm,fontStyle:"italic",marginBottom:18,letterSpacing:1}}>Top 1% global benchmark · Computed live from {sel.length} selected campaign{sel.length===1?"":"s"} · {df} to {dt}</div>

                {/* Structure snapshot */}
                <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:10,marginBottom:22}}>
                  {[
                    ["CAMPAIGNS",sel.length,P.cyan],
                ...
                {/* 10x play */}
                <div style={{padding:"18px 20px",background:"linear-gradient(135deg,"+P.ember+"15,"+P.blaze+"08)",border:"1px solid "+P.ember+"40",borderRadius:14}}>
                  <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}>
                    <span style={{background:"linear-gradient(135deg,"+P.ember+","+P.blaze+")",color:"#fff",padding:"4px 10px",borderRadius:6,fontSize:11,fontWeight:900,fontFamily:fm,letterSpacing:2}}>10X PLAY</span>
                    <span style={{fontSize:12,color:P.ember,fontFamily:fm,fontWeight:800,letterSpacing:1.5,textTransform:"uppercase"}}>The structural rebuild · 60-90 days</span>
                  </div>
                  <ul style={{margin:0,padding:"0 0 0 22px",fontSize:13,color:P.txt,fontFamily:ff,lineHeight:1.85}}>
                ...
                <div style={{marginTop:18,paddingTop:14,borderTop:"1px dashed "+P.rule,fontSize:10,color:P.caption,fontFamily:fm,fontStyle:"italic",lineHeight:1.6}}>
                  This recommendation refreshes every time you change the campaign selection or date range. It mirrors how a top-1% buy-side analyst would read this account — focused on structural levers, not tactical knobs. The 5x play targets visible efficiency gains inside 14 days; the 10x play is the 60-90 day restructure that compounds over the year.
                </div>
              </div>
            </div>;
          })()}
